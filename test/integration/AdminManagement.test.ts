...
describe("Admin Management Integration Tests", function () {
  ...
  async function deployContractsFixture() {
    const [owner, admin1, admin2, nonAdmin, randomUser] =
  ...
  describe("Admin Permissions Across Contracts", function () {
    it("should consistently enforce admin permissions across all contracts", async function () {
      ...
      const voterAdminHash = await voterDatabase.write.addAdmin([
        admin1.account.address,
      ]);
      await publicClient.waitForTransactionReceipt({ hash: voterAdminHash });

      const candidateAdminHash = await candidateDatabase.write.addAdmin([
        admin1.account.address,
      ]);
      await publicClient.waitForTransactionReceipt({
        hash: candidateAdminHash,
      });

      const electionAdminHash = await electionDatabase.write.addAdmin([
        admin1.account.address,
      ]);
      ...
      const voter2AdminHash = await voterDatabase.write.addAdmin([
      ...
      // VoterDatabase
      assert.equal(
        await voterDatabase.read.isAdmin([owner.account.address]),
        true,
        "Owner should be admin of VoterDatabase"
      );
      assert.equal(
        await voterDatabase.read.isAdmin([admin1.account.address]),
        true,
        "Admin1 should be admin of VoterDatabase"
      );
      assert.equal(
      ...
      assert.equal(
        await voterDatabase.read.isAdmin([nonAdmin.account.address]),
        false,
        "NonAdmin should not be admin of VoterDatabase"
      );

      // CandidateDatabase
      assert.equal(
        await candidateDatabase.read.isAdmin([owner.account.address]),
        true,
        "Owner should be admin of CandidateDatabase"
      );
      assert.equal(
      ...
      // ElectionDatabase
      assert.equal(
        await electionDatabase.read.isAdmin([owner.account.address]),
        true,
        "Owner should be admin of ElectionDatabase"
      );
      assert.equal(
    ...
    it("should handle admin removal correctly", async function () {
      const { voterDatabase, admin1, publicClient } = await loadFixture(
        deployContractsFixture
      );

      // Add admin1 as admin
      const addHash = await voterDatabase.write.addAdmin([
        admin1.account.address,
      ]);
      ...
      // Remove admin1
      const removeHash = await voterDatabase.write.removeAdmin([
        admin1.account.address,
      ]);
      ...
      assert.equal(
        await voterDatabase.read.isAdmin([admin1.account.address]),
        false,
        "Admin1 should no longer be admin"
      );

      // Verify admin count has decreased
      const adminCountAfter = await voterDatabase.read.getAdminCount();
      assert.equal(
        Number(adminCountBefore) - 1,
        Number(adminCountAfter),
        "Admin count should decrease by 1"
      );
  ...
  describe("Cross-Contract Admin Operations", function () {
    it("should allow ElectionDatabase with admin rights to modify VoterDatabase", async function () {
      const { voterDatabase, electionDatabase, nonAdmin, publicClient } =
        await loadFixture(deployContractsFixture);

      // Give ElectionDatabase admin rights to VoterDatabase
      const adminHash = await voterDatabase.write.addAdmin([
        electionDatabase.address,
      ]);
      await publicClient.waitForTransactionReceipt({ hash: adminHash });

      // Verify the admin status
      assert.equal(
        await voterDatabase.read.isAdmin([electionDatabase.address]),
        true,
        "ElectionDatabase should be admin of VoterDatabase"
      );

      // Create an election
      const elecHash = await electionDatabase.write.adminCreateElection([
        "Test Election",
        "Election for testing admin operations",
      ]);
      await publicClient.waitForTransactionReceipt({ hash: elecHash });
    ...
    it("should handle complex admin hierarchies correctly", async function () {
      ...
      const admin1CandHash = await candidateDatabase.write.addAdmin([
        admin1.account.address,
      ]);
      ...
      const admin1ElecHash = await electionDatabase.write.addAdmin([
        admin1.account.address,
      ]);
