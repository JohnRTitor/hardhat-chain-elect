...
describe("Import Functionality Tests", function () {
  ...
  async function deployMultipleDatabasesFixture() {
    ...
    const destCandidateDb = await hre.viem.deployContract(
      "CandidateDatabase",
      []
    );
  ...
  describe("Voter Import Functionality", function () {
    it("should allow importing a single voter", async function () {
      const { sourceVoterDb, destVoterDb, voter1, publicClient } =
      ...
      const voterDetails = await destVoterDb.read.adminGetVoterDetails([
        voter1.account.address,
      ]);
      assert.equal(
        voterDetails[0],
        "John Source",
        "Voter name should match source"
      );
      assert.equal(
        voterDetails[1],
        getDobEpochFromAge(30),
        "Voter DoB should match source"
      );
      assert.equal(
        Number(voterDetails[2]),
        GenderEnum.MALE,
        "Voter gender should match source"
      );
      assert.equal(
        voterDetails[3],
        "123 Source St",
        "Voter address should match source"
      );
      assert.equal(
        voterDetails[4],
        "john@source.com",
        "Voter email should match source"
      );
      assert.equal(
        voterDetails[5],
        0n,
        "Voter voting status should match source"
      );
    });

    it("should allow batch importing voters", async function () {
      const { sourceVoterDb, destVoterDb, voter1, voter2, publicClient } =
        await loadFixture(deployMultipleDatabasesFixture);
      ...
      const voter2Details = await destVoterDb.read.adminGetVoterDetails([
        voter2.account.address,
      ]);
      assert.equal(
        voter2Details[0],
    ...
    it("should allow importing all voters", async function () {
      const { sourceVoterDb, destVoterDb, publicClient } = await loadFixture(
        deployMultipleDatabasesFixture
      );
      ...
      assert.equal(
        voterCount,
        sourceVoterCount,
        "All voters should have been imported"
      );
    ...
    it("should handle duplicate imports correctly", async function () {
      ...
      assert.equal(
        finalCount,
        initialCount,
        "No new voters should have been imported"
      );
    });
  });
  ...
  describe("Candidate Import Functionality", function () {
    it("should allow importing a single candidate", async function () {
      const { sourceCandidateDb, destCandidateDb, candidate1, publicClient } =
        await loadFixture(deployMultipleDatabasesFixture);
      ...
      const candidateDetails = await destCandidateDb.read.getCandidateDetails([
        candidate1.account.address,
      ]);
      assert.equal(
        candidateDetails[0],
        "Alice Source",
        "Candidate name should match source"
      );
      assert.equal(
        candidateDetails[1],
        getDobEpochFromAge(35),
        "Candidate DoB should match source"
      );
      assert.equal(
        Number(candidateDetails[2]),
        GenderEnum.FEMALE,
        "Candidate gender should match source"
      );
      assert.equal(
        candidateDetails[3],
        "789 Source Blvd",
        "Candidate address should match source"
      );
      assert.equal(
        candidateDetails[4],
        "alice@source.com",
        "Candidate email should match source"
      );
      assert.equal(
        candidateDetails[5],
        "PhD in Source Studies",
        "Candidate qualifications should match source"
      );
      assert.equal(
        candidateDetails[6],
        "Source for all!",
        "Candidate manifesto should match source"
      );
    });
    ...
    it("should allow batch importing candidates", async function () {
      ...
      assert.equal(
        candidateCount,
        2n,
        "Candidate count should be 2 after import"
      );

      // Check if candidate data is correct
      const candidate2Details = await destCandidateDb.read.getCandidateDetails([
        candidate2.account.address,
      ]);
      assert.equal(
        candidate2Details[0],
    ...
    it("should allow importing all candidates", async function () {
      const { sourceCandidateDb, destCandidateDb, publicClient } =
        await loadFixture(deployMultipleDatabasesFixture);
      ...
      const sourceCandidateCount =
        await sourceCandidateDb.read.getCandidateCount();
      assert.equal(
        candidateCount,
        sourceCandidateCount,
        "All candidates should have been imported"
      );
    ...
    it("should handle duplicate candidate imports correctly", async function () {
      ...
      const importHash2 =
        await destCandidateDb.write.adminBatchImportCandidates([
          sourceCandidateDb.address,
      ...
      assert.equal(
        finalCount,
        initialCount,
        "No new candidates should have been imported"
      );
    });
  });
  ...
  describe("Import Error Handling", function () {
    it("should handle importing from invalid source contracts", async function () {
      const { destVoterDb, destCandidateDb, voter1, candidate1 } =
        await loadFixture(deployMultipleDatabasesFixture);
