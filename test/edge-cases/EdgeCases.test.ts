...
describe("Edge Cases Tests", function () {
  ...
  async function deployContractsFixture() {
    const [owner, voter1, voter2, candidate1, candidate2, attacker] =
  ...
  describe("Boundary Conditions", function () {
    ...
    it("should handle extremely old ages correctly", async function () {
      const { candidateDatabase, candidate1, publicClient } = await loadFixture(
        deployContractsFixture
      );
      ...
      assert.equal(
        isRegistered,
        true,
        "Very old candidate should be registered"
      );
    });
    ...
    it("should handle elections with maximum number of candidates", async function () {
      ...
      assert.equal(
        ...
        numCandidates,
        `All ${numCandidates} candidates should be enrolled`
      );
    });
  });
  ...
  describe("Invalid Operations", function () {
    it("should prevent re-opening an already open election", async function () {
      ...
      const elecHash = await electionDatabase.write.adminCreateElection([
        "Duplicate Open Test",
        "Testing duplicate open operations",
      ]);
      await publicClient.waitForTransactionReceipt({ hash: elecHash });
  ...
  describe("Race Conditions and Multiple Updates", function () {
    it("should handle sequential updates to voter information correctly", async function () {
      ...
      assert.equal(
        finalDetails[0],
        lastUpdate.name,
        "Name should match last update"
      );
      assert.equal(
        finalDetails[1],
        getDobEpochFromAge(lastUpdate.age),
        "Age should match last update"
      );
      assert.equal(
        Number(finalDetails[2]),
        lastUpdate.gender,
        "Gender should match last update"
      );
      assert.equal(
        finalDetails[3],
        lastUpdate.address,
        "Address should match last update"
      );
      assert.equal(
        finalDetails[4],
        lastUpdate.email,
        "Email should match last update"
      );
    });
    ...
    it("should handle tie votes correctly", async function () {
      ...
      const vote1Hash = await electionDatabase.write.vote(
      ...
      const vote2Hash = await electionDatabase.write.vote(
      ...
      // Verify vote counts
      const votes1 = await electionDatabase.read.getVotesOfCandidate([
        0n,
        candidate1.account.address,
      ]);
      const votes2 = await electionDatabase.read.getVotesOfCandidate([
        0n,
  ...
  describe("Zero Address and Deployment Edge Cases", function () {
    ...
    it("should handle zero address checks in admin functions", async function () {
      const { voterDatabase, candidateDatabase } = await loadFixture(
        deployContractsFixture
      );
